...
if [[ "$TERM" != "dumb" ]] && [[ "$DISABLE_LS_COLORS" != "true" ]]; then
    PROMPT='
%{$fg[red]%}%n%{$reset_color%}@%{$fg[green]%}%m%{$reset_color%}:%{$fg[blue]%}%~%{$reset_color%} %{$fg[yellow]%}%w %T%{$fg[magenta]%} %!%{$reset_color%} $(battery_pct_prompt)
$(myjobs)%(!.%{$fg[red]%}#.%{$fg[cyan]%}❯)%{$reset_color%} '
    ...
    ZSH_THEME_GIT_PROMPT_DIRTY="%{$fg[white]%}[%{$fg[yellow]%}⚡%{$fg[white]%}]%{$reset_color%}"
    ZSH_THEME_GIT_PROMPT_CLEAN="%{$fg[white]%}[%{$fg[green]%}✓%{$fg[white]%}]%{$reset_color%}"
    ...
    function my_git_prompt_status_and_time() {
        ...
        if [[ -n $NOW_GIT_PROMPT_STATUS ]]; then 
            echo "%{$fg[white]%}[$NOW_GIT_PROMPT_STATUS%{$fg[white]%}]$NOW_GIT_TIME_SINCE%{$reset_color%}"
        else
            if [[ -n $NOW_GIT_TIME_SINCE ]]; then
                echo "%{$fg[white]%}‖$NOW_GIT_TIME_SINCE%{$reset_color%}"
            fi
        fi
    }
